...
With the following table \@ref(tab:Table-Arrestation-2), we have a look on those extreme value and about the more current profile of the arrested.
The  standard profile is a young white male, with drogs or alcohol present.
...
But unfortunately we are not satisfied that our model. Because we noticed that the residues are very high and that our data do not follow a normal dsitribution. Therefore, now we are make the same model, but assuming a poisson distribution.

### With a poisson distribution 

Normal distribution describes continuous data which have a symmetric distribution, with a characteristic 'bell' shape, which is not the case in our data, as we observed in the figure \@ref(fig:Residuals). Therefore we made a mistake, therefore in the following part, we are going to use the same model but with a poisson distribution.

```{r model_arrestation_poisson}
...
We note that all our estimate are significant with 1% except for the variable race as we observe in the table  \@ref(tab:Table-estimate_poisson). 

We note that with a normal distribution (\@ref(tab:Table-estimate)) the sginifcant variables were the same.
The change is for the intercept, which is slightly smaller than previously, 3.84 instead of 49.24. The effect of the estimate are the same. Ans as previsouly, variable race isn't significant. 


```{r Table-estimate_poisson,warning=FALSE, message=FALSE, echo = FALSE}
...
In the fifure, \@ref(tab:Table-estimate_poisson) we copare the distribution of the error, one with a poisson distribution and the other with a normal distribution. We note that the poisson distribution is cleraly more adapted for our data. And we we have a zoom of this new residuals distribution, we note our residuals find themselves in an interval of [-10,10], which is pretty good.

```{r Residuals_poisson, fig.cap="Residuals of the models about the arrested", warning=FALSE, message=FALSE, echo = FALSE}
...
Again, after removing the extrme value, we compare the residuals between the 2 distributions, as we can observe in the \@ref(tab:Residuals-2_poisson). And we come to the same conclusion as before. The residuals as less important for the poisson distribution. The residuals are clearly centered around 0 

```{r Residuals-2_poisson, fig.cap="Residuals of the models about the arrested", warning=FALSE, message=FALSE, echo = FALSE}

p9<- augment_model_poisson %>%
  filter(arrestation<100) %>%
  ggplot(aes(x=arrestation,y=.resid)) + 
  geom_hline(yintercept = 0, colour = "bisque3", size= 1) + 
  geom_line() + 
  labs(
    x = "Arrestation", 
    y = "Residuals",
    title = "More centered around 0",
    subtitle = "Poisson distribution ")+
  scale_y_continuous(limits  = c(-25,50))

p18<- augment_model_poisson %>%
  filter(arrestation<100) %>%
  ggplot(aes(x=arrestation,y=.resid)) + 
  geom_hline(yintercept = 0, colour = "bisque3", size= 1) + 
  geom_line() + 
  labs(
    x = "Arrestation", 
    y = "Residuals",
    title = "Zoom on the residuals disribution",
    subtitle = "Poisson distribution ")
...
Compare to our previous model with less variables, the significance of the estimate does not change. But we note that the type of arrestation "taken into custody" and being "unarmed" are signidicant with 5%, as we observe in the table \@ref(tab:Table-Arrestation-2).
...
```{r Residuals-model-victim, fig.cap="Dstribution of the residuals", warning=FALSE, message=FALSE, echo = FALSE}
p7<- augment_model_victim %>% 
filter(Victim<200) %>%
  ggplot(aes(x=Victim,y=.resid)) + 
  geom_hline(yintercept = 0, colour = "bisque3", size= 1) + 
  geom_line() + 
...
(p7 + p8) +
  plot_annotation(
    title = "Without the extreme value")
...
```{r Table-estimate-victim_poisson,warning=FALSE, message=FALSE, echo = FALSE}
...
Removing the extreme value, again we note that with a poisson distribution, it is more correct, 

```{r Residuals-2-poisson-victim, fig.cap="Residuals of the models about the arrested", warning=FALSE, message=FALSE, echo = FALSE}

p14<- augment_model_victim_poisson %>% 
filter(Victim<200) %>%
  ggplot(aes(x=Victim,y=.resid)) + 
  geom_hline(yintercept = 0, colour = "bisque3", size= 1) + 
  geom_line() + 
...
p16<- augment_model_victim_poisson %>% 
filter(Victim<200) %>%
  ggplot(aes(x=Victim,y=.resid)) + 
  geom_hline(yintercept = 0, colour = "bisque3", size= 1) + 
  geom_line() + 
...
```{r Prediction-model-race, fig.cap="Race prediction model", warning=FALSE, message=FALSE, echo = FALSE}
...
We have to look at the sensitivity (ability to predict true positive, here black people) and specificity (ability to predict true negative, here whites), as well the accuracy: if our model well predicts in average the outcome. presented in the table \@ref(tab:Table-Arrestation-2). Here, the accuracy is a little bit better than hazard (0.6611 against 0.5, considered here as hazard since we have only two outcomes). The sensitivity (.7560) is quite high but specificity is low (.5578). Our model predicts well black but have more difficulty to predict white, which was the issue we faced with our other two models: the white race level is non-significant.
