...
p1<- augment_model %>% 
  ...
  labs(
    x = "Arrestation", 
    y = "Residuals",
    title = "Bad prediction\nWhen the number of arrestation increase",
...
p12<-augment_model %>%
  ...
  labs(
    x = "Arrestation", 
    y = "Residuals",
    title = "Bad prediction\nWhen the number of arrests increases",
...
In the next model, we add the type of arrest and whether the criminal is armed or not, in order to explain our response variable "Arrest".
...
p3<- augment_model_victim %>% 
  ...
  labs(
    x = "Number of victims", 
    y = "Residuals",
    title = "Bad prediction\nWhen the number of victim increases",
    subtitle = "Because of the extreme value ")
...
We have to look at the sensitivity (ability to predict true positive, here black people) and specificity (ability to predict true negative, here whites), as well the accuracy: if our model well predicts in average the outcome. Here, the accuracy is a little bit better than hazard (0.6611 against 0.5, considered here as hazard since we have only two outcomes). The sensitivity (.7560) is quite high but specificity is low (.5578). Our model predicts well black but have more difficulty to predict white, which was the issue we faced with our other two models: the white race level is non-significant.

Again, the model 


```{r Prediction-model-race, fig.cap="Race prediction model", warning=FALSE, message=FALSE, echo = FALSE}
## Create the training/test set after modfying the data
...
Euge<-confusionMatrix(data=pred.te, reference = arrest.te$arrestees_race) 
